...
//        TLRPC.StickerSet stickerSet = messages_stickerSet.set;
    ...
    public void loadMedia(long dialogId, int count, int max_id, int min_id, int type, long topicId, int fromCache, int classGuid, int requestIndex, boolean skipPhotos) {
        loadMedia(dialogId, count, max_id, min_id, type, topicId, fromCache, classGuid, requestIndex, skipPhotos, null, null);
    }
    public void loadMedia(long dialogId, int count, int max_id, int min_id, int type, long topicId, int fromCache, int classGuid, int requestIndex, ReactionsLayoutInBubble.VisibleReaction tag, String query) {
        loadMedia(dialogId, count, max_id, min_id, type, topicId, fromCache, classGuid, requestIndex, false, null, null);
    }

    public void loadMedia(long dialogId, int count, int max_id, int min_id, int type, long topicId, int fromCache, int classGuid, int requestIndex, boolean skipPhotos, ReactionsLayoutInBubble.VisibleReaction tag, String query) {
    ...
    private void processLoadedMedia(TLRPC.messages_Messages res, long dialogId, int count, int max_id, int min_id, int type, long topicId, int fromCache, int classGuid, boolean isChannel, boolean topReached, int requestIndex, boolean skipPhotos) {
        ...
        if (fromCache != 0 && res != null && res.messages != null && ((res.messages.isEmpty() && min_id == 0) || (res.messages.size() <= 1 && min_id != 0)) && !DialogObject.isEncryptedDialog(dialogId)) {
            ...
            loadMedia(dialogId, count, max_id, min_id, type, topicId, 0, classGuid, requestIndex, skipPhotos, null, null);
        } else {
            if (fromCache == 0) {
    ...
    private void loadMediaDatabase(long uid, int count, int max_id, int min_id, int type, long topicId, ReactionsLayoutInBubble.VisibleReaction tag, int classGuid, boolean isChannel, int fromCache, int requestIndex) {
        loadMediaDatabase(uid, count, max_id, min_id, type, topicId, tag, classGuid, isChannel, fromCache, requestIndex, false);
    }
    ...
    public void loadMusic(long dialogId, long maxId, long minId) {
        getMessagesStorage().getStorageQueue().postRunnable(() -> {
            ...
            AndroidUtilities.runOnUIThread(() -> getNotificationCenter().postNotificationName(NotificationCenter.musicDidLoad, dialogId, arrayListBegin, arrayListEnd, NekoXConfig.getLastMusicMessageId()));
        });
    }
    //---------------- MEDIA END ----------------
...
//            }
    ...
    public boolean saveToRingtones(TLRPC.Document document) {
        ...
        if (document.size > MessagesController.getInstance(currentAccount).ringtoneSizeMax) {
            NotificationCenter.getGlobalInstance().postNotificationName(NotificationCenter.showBulletin, Bulletin.TYPE_ERROR_SUBTITLE, LocaleController.formatString(R.string.TooLargeError), LocaleController.formatString(R.string.ErrorRingtoneSizeTooBig, (MessagesController.getInstance(UserConfig.selectedAccount).ringtoneSizeMax / 1024)));
            return false;
        }
        for (int a = 0; a < document.attributes.size(); a++) {
            TLRPC.DocumentAttribute attribute = document.attributes.get(a);
            if (attribute instanceof TLRPC.TL_documentAttributeAudio) {
                if (attribute.duration > MessagesController.getInstance(currentAccount).ringtoneDurationMax) {
                    NotificationCenter.getGlobalInstance().postNotificationName(NotificationCenter.showBulletin, Bulletin.TYPE_ERROR_SUBTITLE, LocaleController.formatString(R.string.TooLongError), LocaleController.formatString(R.string.ErrorRingtoneDurationTooLong, MessagesController.getInstance(UserConfig.selectedAccount).ringtoneDurationMax));
                    return false;
                }
            }
        }
