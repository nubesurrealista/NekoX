...
//        items.add(new Item(4, LocaleController.getString(R.string.NewChannel), newChannelIcon));
        ...
        if (NekoConfig.useProxyItem.Bool() && (!NekoConfig.hideProxyByDefault.Bool() || SharedConfig.isProxyEnabled())) {
            items.add(new CheckItem(13, LocaleController.getString(R.string.Proxy), R.drawable.baseline_security_24, SharedConfig::isProxyEnabled, () -> {
    ...
    public class CheckItem extends Item {
        ...
        public void bindCheck(DrawerActionCheckCell actionCell) {
            actionCell.setTextAndValueAndCheck(String.valueOf(text), icon, null, isChecked.invoke(), false, false);
            if (doSwitch != null) {
