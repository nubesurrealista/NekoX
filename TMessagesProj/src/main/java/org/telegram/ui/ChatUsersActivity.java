    ...
    private void updateRows() {
        ...
        if (type == TYPE_KICKED) {
            ...
            if (!transfer && ChatObject.isChannel(currentChat) && currentChat.megagroup && !currentChat.gigagroup && (info == null || info.participants_count <= 200 || !isChannel && info.can_set_stickers)) {
                // 030: commented out in upstream on 10.12.0
                if (ChatObject.hasAdminRights(currentChat)) recentActionsRow = rowCount++;
...
//            }
    ...
    public View createView(Context context) {
        ...
        if (selectType != SELECT_TYPE_MEMBERS || type == TYPE_USERS || ChatObject.hasAdminRights(currentChat) && (type == TYPE_BANNED || type == TYPE_KICKED)) {
            ...
            if (type == TYPE_KICKED) {
                doneItem = menu.addItemWithWidth(done_button, R.drawable.ic_ab_done, dp(56), getString(R.string.Done));
            }
                ...
                if (selectType != SELECT_TYPE_MEMBERS) {
                    if (selectType == SELECT_TYPE_EXCEPTION || selectType == SELECT_TYPE_ADMIN) {
                        if (selectType != SELECT_TYPE_ADMIN && canEditAdmin && (participant instanceof TLRPC.TL_channelParticipantAdmin || participant instanceof TLRPC.TL_chatParticipantAdmin)) {
                            ...
                            builder.setMessage(LocaleController.formatString(R.string.AdminWillBeRemoved, UserObject.getUserName(user)));
            ...
            switch (holder.getItemViewType()) {
                ...
                case VIEW_TYPE_INNER_CHECK:
                    ...
                    if (position == sendMediaPhotosRow) {
                        ...
                        checkBoxCell.setText(getString(R.string.SendMediaEmbededLinks), "", !defaultBannedRights.embed_links && !defaultBannedRights.send_plain, true, animated);
                    } else if (position == sendReactionsRow) {
                        checkBoxCell.setText(getString(R.string.UserRestrictionsSendReactions), "", !defaultBannedRights.send_reactions, false);
                    } else if (position == sendPollsRow) {
                        checkBoxCell.setText(getString(R.string.SendMediaPolls), "", !defaultBannedRights.send_polls, true, animated);
                    } else
