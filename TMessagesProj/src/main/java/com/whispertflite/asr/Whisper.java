...
public class Whisper {
    ...
    public void processSamples(float[] samples) {
        Thread processBufferThread = new Thread(() -> processRecordBufferLoop(samples));
        processBufferThread.start();
    }
    ...
    private void processRecordBufferLoop(float[] samples) {
    ...
    private void processRecordBuffer(float[] samples) {
        try {
            if (mWhisperEngine.isInitialized()) {
                ...
                synchronized (mWhisperEngine) {
                    whisperResult = mWhisperEngine.process(mAction, mLangToken, samples);
                }
                sendResult(whisperResult);

                long timeTaken = System.currentTimeMillis() - startTime;
                Log.d(TAG, "Time Taken for transcription: " + timeTaken + "ms, result: " + whisperResult.getResult());
