...
object DnsFactory {
    ...
    fun providers() = if (MomoConfig.customDoH.String().isNotBlank()) arrayOf(
        MomoConfig.customDoH.String())
    else arrayOf(
    ...
    fun lookup(domain: String, fallback: Boolean = false): List<InetAddress> {
        ...
        if (!MomoConfig.useSystemDNS.Bool()) {
            ...
            } else if (MomoConfig.useIPv6.Bool() xor !fallback) Type.A else Type.AAAA
